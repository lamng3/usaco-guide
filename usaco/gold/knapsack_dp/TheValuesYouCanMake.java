...
/**
Source: CF
Problem url: https://codeforces.com/contest/687/problem/C

naive idea: generate all possible subsets that sum up to k, then perform knapsack dp on each subset
    * obviously, this TLE: https://ideone.com/UjtA3E

hint: 
dp[i][j] = whether can find a set of coins sum up to i and another set sum up to j
answer will be all values x such that dp[x][k-x] is true
at each coin, we can either include in Arya, or include in the other set
...
public class TheValuesYouCanMake {
    public static void solve(FastScanner io) throws Exception {
        ...
        for (int i = 0; i <= k; i++) {
            if (dp[i][k-i]) answer.add(i);
        }
        io.println(answer.size());
        for (int x : answer) io.print(x + " ");
    }
