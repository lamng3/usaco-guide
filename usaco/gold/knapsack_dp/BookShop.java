...
public class BookShop {
    public static void solve(FastScanner io) throws Exception {
        // state: current price, maximum number of pages
        int n = io.nextInt(), x = io.nextInt();
        
        int[] h = new int[n+1];
        int[] s = new int[n+1];
        for (int i = 1; i <= n; i++) h[i] = io.nextInt();
        for (int i = 1; i <= n; i++) s[i] = io.nextInt();

        int[][] dp = new int[n+1][x+1];
        // dp[i][j] = maximum number of pages can buy from [0..i] with at most j 
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= x; j++) {
                dp[i][j] = dp[i-1][j]; // do not buy
                if (j < h[i]) continue;
                dp[i][j] = Math.max(dp[i][j], dp[i-1][j-h[i]] + s[i]);
            }
        }
        io.println(dp[n][x]);
    }
