...
public class FruitFeast {
    public static void solve(FastScanner io) throws Exception {
        int t = io.nextInt(), a = io.nextInt(), b = io.nextInt();
        
        boolean[] dp = new boolean[t+1];
        dp[0] = true;
        for (int i = a; i <= t; i++) dp[i] |= dp[i-a];
        for (int i = b; i <= t; i++) dp[i] |= dp[i-b];

        // bessie can drink water 1 time
        for (int i = 0; i <= t; i++) {
            if (dp[i]) dp[(int)(i/2)] = true;
        }
        // fill again
        for (int i = a; i <= t; i++) dp[i] |= dp[i-a];
        for (int i = b; i <= t; i++) dp[i] |= dp[i-b];
    ...
    */
    public static void main(String[] args) throws Exception {
        FastScanner io = new FastScanner("feast"); // usaco
