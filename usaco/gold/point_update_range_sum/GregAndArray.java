...
A classic problem but requires some meticulous thinking at first.
Setup a Fenwick Tree that supports RUPQ (range update point query).
Use difference-array trick to perform queries counting.
...
public class GregAndArray {
    static class RangeFenwickTree {
        ...
        void range_add(int l, int r, long u) {
            add(l, u);
            add(r+1, -u);
        }
        long sum(int k) {
    ...
    public static void solve(FastScanner io) throws Exception {
        ...
        for (int i = 1; i <= m; i++) {
            ...
            long d = io.nextLong();
            ops[i] = new Operation(l, r, d);
        }

        // count number of times an operation is applied
        ...
        for (int i = 1; i <= m; i++) count[i] = count[i-1] + pref[i];

        for (int i = 1; i <= m; i++) {
            if (count[i] == 0) continue;
            Operation op = ops[i];
            ft.range_add(op.l, op.r, op.d * count[i]);
        }
