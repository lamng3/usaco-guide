...
public class DynamicRangeMinimumQueries {
    static class SegmentTree {
        long[] st, a;
        public SegmentTree(int n, long[] x) {
            st = new long[4*n];
            a = x;
            build(0, 0, n-1);
        }
        ...
        void update(int v, int tl, int tr, int pos, int u) {
            if (tl == tr) {
                st[v] = u;
                return;
            }
        ...
        long min(int v, int tl, int tr, int l, int r) {
            if (l > r) return INF;
            if (l <= tl && tr <= r) return st[v];
            int tm = (tl + tr) / 2;
            return Math.min(
                min(v*2+1, tl, tm, l, Math.min(r, tm)),
                min(v*2+2, tm+1, tr, Math.max(l, tm+1), r)
            );
        } 
    }
    ...
    public static void solve(FastScanner io) throws Exception {
        ...
        long[] x = new long[n];
        for (int i = 0; i < n; i++) x[i] = io.nextLong();
        SegmentTree st = new SegmentTree(n, x);
        for (int i = 0; i < q; i++) {
            int t = io.nextInt();
            if (t == 1) {
