package usaco.gold.paths_on_grids;
/**
Source: CSES
Problem url: https://cses.fi/problemset/task/1746/

Not sure what I was thinking 4 months ago on this problem :)
But this is surely a redemption on that. 
Thinking back, it was a straight forward flow.

dp[i][j] = counting the number of arrays from [0..i] with x[i] = j
if x[i] = 0
...
Notes:
    - Make sure the bounds of i and j are correct. 
    - Perform modular. 
    - Set everything to Long.
...
public class ArrayDescription {
    public static void solve(FastScanner io) throws Exception {
        // dp[i][j] = count number of arrays from [0..i] with x[i] = j
        int mod = mod_prime;
        ...
        for (int i = 1; i < n; i++) {
            if (x[i] == 0) {
                ...
                if (x[i] > 1) dp[i][x[i]] += dp[i-1][x[i]-1] % mod;
                if (x[i] < m) dp[i][x[i]] += dp[i-1][x[i]+1] % mod;
                dp[i][x[i]] %= mod;
            }
        }
