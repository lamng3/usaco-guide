package usaco.silver.prefix_sums;
/**
Source: USACO
Problem url: https://usaco.org/index.php?page=viewproblem2&cpid=691
*/
...
*/
public class HPS {
    private static int to_idx(char g) {
        switch (g) {
            case 'H': return 0;
            case 'P': return 1;
            case 'S': return 2;
    ...
    public static void solve(FastScanner io) throws Exception {
        ...
        for (int i = 1; i <= N; i++) {
            char g = io.nextLine().toCharArray()[0];
            pref[i] = pref[i-1].clone();
            pref[i][to_idx(g)]++;
        }
        int answer = 0;
        for (int i = 1; i <= N; i++) {
            int[] first = pref[i].clone();
            int[] second = pref[N].clone();
            for (int j = 0; j < 3; j++) second[j] -= pref[i][j];
            int optimal = best(first) + best(second);
            int bessie = Math.max(best(pref[N]), optimal);
    ...
    */
    public static void main(String[] args) throws Exception {
        FastScanner io = new FastScanner("hps"); // usaco
