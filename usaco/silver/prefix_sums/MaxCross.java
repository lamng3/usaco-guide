...
/**
Source: USACO
Problem url: https://usaco.org/index.php?page=viewproblem2&cpid=715
*/
...
public class MaxCross {
    public static void solve(FastScanner io) throws Exception {
        ...
        int[] s = new int[N+1];
        for (int i = 0; i < B; i++) s[io.nextInt()]++;
        int answer = oo_int;
        int[] broken = new int[N+1];
        for (int i = 1; i <= N; i++) {
            broken[i] = broken[i-1] + s[i];
            if (i >= K) answer = Math.min(answer, broken[i] - broken[i-K]);
        }
        io.println(answer);
    }
    ...
    */
    public static void main(String[] args) throws Exception {
        FastScanner io = new FastScanner("maxcross"); // usaco
