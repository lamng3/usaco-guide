...
public class Div7 {
    public static void solve(FastScanner io) throws Exception {
        ...
        int[] cow = new int[N+1];
        for (int i = 1; i <= N; i++) cow[i] = io.nextInt();
        ...
        for (int i = 1; i <= N; i++) {
            pref[i] = pref[i-1] + cow[i];
            pref[i] %= mod;
            if (prev.containsKey(pref[i])) {
                answer = Math.max(answer, i - prev.get(pref[i]));
            }
            else prev.put(pref[i], i);
        }
    ...
    */
    public static void main(String[] args) throws Exception {
        FastScanner io = new FastScanner("div7"); // usaco
