...
public class GoodSubarrays {
    public static void solve(FastScanner io) throws Exception {
        // find j where pref[i] - pref[j-1] = i - j + 1
        // pref[i] - i = pref[j-1] - (j-1)
        // find previous k such that pref[k] - k = pref[i] - i
        // long type for large n
        int n = io.nextInt();
        char[] s = io.nextLine().toCharArray();
        int[] a = new int[n+1];
        for (int i = 1; i <= n; i++) a[i] = s[i-1] - '0';
        Map<Long, Long> count = new HashMap<>();
        count.put(0L, 1L); // empty subarray
        long cursum = 0, answer = 0;
        for (int i = 1; i <= n; i++) {
            cursum += a[i];
            answer += count.getOrDefault(cursum - i, 0L);
            count.merge(cursum - i, 1L, Long::sum);
        }
        io.println(answer);
    }
